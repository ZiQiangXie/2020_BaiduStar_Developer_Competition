...
add_arg('test_pic_path', str, None, "path of test pic")
# yapf: enable
...
def eval(args):
    ...
    l = np.hstack(l)
    s = np.hstack(s)
    res_final, cos_final = post_process(f, g, l, s, args.thresh, k=args.top_k)

    output_path = args.output_path
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    print("Saving result to {}".format(output_path))
    save_result(res_final, cos_final, output_path, args.detect_path, args.test_pic_path)
    sys.stdout.flush()
